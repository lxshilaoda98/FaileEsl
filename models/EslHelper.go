			...
			case "CHANNEL_CREATE":
				...
				if msg.Headers["variable_sofia_profile_name"] == "internal" && msg.Headers["variable_direction"] == "outbound" && msg.Headers["Caller-ANI"] == "0000000000" && msg.Headers["Answer-State"] == "ringing" {
					...
					if Istrasfer > 0 {
						...
						_, err := db.SqlDB.Query("update call_userstatus set CallerNumber =? ,ChannelUUid=?,TPRingTime=Now(),CallStatus='话机振铃',CallType='out' where ChannelUUid=?", CallModel.CalledNumber,
							CallModel.Calluuid, CallModel.Calluuid)
						if err != nil {
